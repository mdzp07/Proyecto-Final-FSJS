...
it("2. POST /api/productos agrega un nuevo producto y devuelve un código de estado 201", async () => {
        ...
        expect(statusCode).toBe(201); // Comprobando que el codigo de estado es 201
    });

})

it("4. PUT /api/productos/:id devuelve un status code 400 al intentar actualizar un id que no coincide con el id dentro del payload", async () => {
...
describe("Operaciones CRUD de Usuarios", () => {
    ...
    it("1. GET /api/usuarios devuelve un status code 200 y el tipo de dato recibido es un arreglo", async () => {
    ...
    it("2. DELETE /api/usuarios/:id obtiene un código 404 al intentar eliminar un usuario con un id que no existe", async () => {
        const { statusCode } = await request(server)
    ...
    it("3. PUT /api/usuarios/:id devuelve un status code 400 al intentar actualizar un id diferente al id dentro del payload", async () => {
